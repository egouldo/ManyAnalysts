...
Note that `timetk::step_box_cox()` directly optimises the estimation of the transformation parameter, $\lambda$, using the "Guerrero" method such that $\lambda$ minimises the coefficient of variation for sub series of a numeric vector (see `?timetk::step_box_cox()`, for further details see @timet).
...
all_model_fits <- 
  model_vars %>% 
  cross_join(., 
             {ManyEcoEvo::MManyEcoEvo_results %>% 
                 select(dataset, 
                        exclusion_set, 
                        estimate_type, 
                        publishable_subset) %>% 
  ...
  left_join(., {ManyEcoEvo::MManyEcoEvo_results %>% 
      select(dataset, 
             exclusion_set, 
             estimate_type, 
             publishable_subset, 
