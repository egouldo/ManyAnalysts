...
all_model_fits <- 
  ...
  mutate(effects_analysis = 
            map(effects_analysis, 
                mutate, 
                weight = importance_weights(1/box_cox_var)), #TODO issue #27 REMOVE weights!!
         effects_analysis = 
           map(effects_analysis, 
               ~ .x %>% 
                 unnest(review_data) %>% 
...
fit_multivar_wrap <- function(data_tbl, ..., env = rlang::caller_env()){
...
We systematically investigated the impact of using different weighting schemes (no weights, inverse-Box-Cox transformed,  and inverse folded variance, of the absolute deviation scores) and random-effects inclusion (effect ID, reviewer ID, or both effect ID and reviewer ID) on model convergence, singularity and other model checking metrics to aid decision-making about the appropriate weighting scheme and random-effects structure for these models.

Note that the folded variance of the absolute deviation scores is calculated during the Box-Cox transformation of the deviation scores from the mean for each effect.
...
# library(mathml)
# mathml::math(call("<-", quote(folded_v(abs_dev_score, VZr)), folded_v))
```


$$
\begin{aligned}
\text{folded}_\mu &= \sigma \sqrt{\frac{2}{\pi}} \exp\left(-\frac{\mu^2}{2 \sigma^2}\right) + \mu \left(1 - 2 \times \boldsymbol{\phi}\left(-\frac{\mu}{\sigma}; 0,1\right)\right) \\
\text{fold_se}_\mu &= \sqrt{\mu^2 + \sigma^2 - \text{folded}_\mu^2} \\
\text{folded}_v &= \text{folded_se}_\mu^2
\end{aligned}
$$ {#eq:folded-variance}
...
- $\text{folded}_\mu$ is the folded mean of the deviation scores from the mean for effect $i$,
- $\text{folded}_v$ is the folded variance of the deviation scores from the mean for effect $i$,
...
- $\sigma$ is the square root of the variance of effect $i$, $VZ_r$.



*weights findings*
We also found that models that used the intended weights were more likely to fail to converge, and that models that used the intended weights were more likely to be singular.


*random effects*
We found that models that included random effects for both Effect ID and Reviewer ID were more likely to fail to converge, and that models that included random effects for Reviewer ID only were more likely to be singular. 