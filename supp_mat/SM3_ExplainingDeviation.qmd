...
ManyEcoEvo_yi_viz %>%
  filter(exclusion_set == "complete", 
         dataset == "blue tit",
         model_name %nin% c("box_cox_rating_cont", "box_cox_rating_cat_no_int","sorensen_glm" )) %>% 
  bind_rows({euc_yi_results %>% 
      filter(model_name %nin% c("box_cox_rating_cont", "box_cox_rating_cat_no_int","sorensen_glm", "uni_mixed_effects"))}) %>% 
  mutate( dataset = case_when(str_detect(dataset, "eucalyptus") ~ "Eucalyptus",
                              TRUE ~ dataset),
          model_name = forcats::as_factor(model_name) %>% 
            forcats::fct_relevel(c("box_cox_rating_cat_no_int", 
                                   "box_cox_rating_cont", 
                                   "sorensen_glm", 
                                   "uni_mixed_effects")) %>% 
