...
#All analyses
All_Analyses_Per_Team <- Master %>% 
  count(dataset, TeamIdentifier, sort = TRUE)
allgrouped <- group_by(All_Analyses_Per_Team,dataset)
All_Total_Analyses <- summarise(allgrouped,
                                totalanalyses=sum(n))
All_Total_Analyses$subset <- rep("all",
                                 nrow(All_Total_Analyses))

All_Teams_Per_Dataset <-All_Analyses_Per_Team %>%  
...
#Effects eval
Effect_Analyses_Per_Team<-effects %>% 
  count(dataset, TeamIdentifier, sort = TRUE)
...
Table3<-filter(Table3,subset!="all")
...
WholeConclusions<-bind_rows(AllConcSummary,EffectsConcSummary,PredictionsConcSummary)
WholeConclusionsWide<-spread(WholeConclusions,Conclusion,count,fill=0)
Table4<-WholeConclusionsWide[c(1,2,3,4,5,6,7,9,8)]



#Sorensen Diversity Index Data
...
#   read_csv(
#     here::here("data-raw/metadata_and_key_data/TeamIdentifierAnonymised.csv")
#   )
