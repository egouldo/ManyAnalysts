...
As described in the summary statistics section of the manuscript, `r Table1 %>% filter(dataset == "blue tit", subset_name == "effects") %>% pluck("teams")` teams submitted `r Table1 %>% filter(dataset == "blue tit", subset_name == "effects") %>% pluck("totalanalyses")` $Z_r$ model estimates and `r Table1 %>% filter(dataset == "blue tit", subset_name == "predictions") %>% pluck("teams")` teams submitted `r Table1 %>% filter(dataset == "blue tit", subset_name == "predictions") %>% pluck("totalanalyses")` $y_i$ predictions for the blue tit dataset. Similarly, `r Table1 %>% filter(dataset == "eucalyptus", subset_name == "effects") %>% pluck("teams")` submitted `r Table1 %>% filter(dataset == "eucalyptus", subset_name == "effects") %>% pluck("totalanalyses")` $Z_r$ model estimates and `r Table1 %>% filter(dataset == "eucalyptus", subset_name == "predictions") %>% pluck("teams")` teams submitted `r Table1 %>% filter(dataset == "eucalyptus", subset_name == "predictions") %>% pluck("totalanalyses")` $y_i$ predictions for the *Eucalytpus* dataset. The majority of the blue tit analyses specified normal error distributions and were non-Bayesian mixed effects models. Analyses of the *Eucalyptus* dataset rarely specified normal error distributions, likely because the response variable was in the form of counts. Mixed effects models were also common for *Eucalytpus* analyses (@tbl-Table1).

```{r}
#| label: tbl-Table1
#| tbl-cap: "Summary of the number of analysis teams, total analyses, models with normal error distributions, mixed effects models, and models developed with Bayesian statistical methods for effect size analyses only ($Z_r$) and out-of-sample prediction only ($y_i$)."
...
The composition of models varied substantially (@tbl-Table2) in regards to the number of fixed and random effects, interaction terms and the number of data points used. For the blue tit dataset, models used up to `r Table2 %>% filter(dataset == "blue tit", variable == "fixed") %>% pluck("max", max)` fixed effects, `r Table2 %>% filter(dataset == "blue tit", variable == "random") %>% pluck("max", max)` random effects, and `r Table2 %>% filter(dataset == "blue tit", variable == "interactions") %>% pluck("max", max)` interaction terms and had sample sizes ranging from `r Table2 %>% filter(dataset == "blue tit", variable == "samplesize") %>% pluck("min", min)` to `r Table2 %>% filter(dataset == "blue tit", variable == "samplesize") %>% pluck("max", max)`. For the *Eucalyptus* dataset models had up to `r Table2 %>% filter(dataset == "eucalyptus", variable == "fixed") %>% pluck("max", max)` fixed effects, `r Table2 %>% filter(dataset == "eucalyptus", variable == "random") %>% pluck("max", max)` random effects, `r Table2 %>% filter(dataset == "eucalyptus", variable == "interactions") %>% pluck("max", max)` interaction terms and sample sizes ranging from `r Table2 %>% filter(dataset == "eucalyptus", variable == "samplesize") %>% pluck("min", min)` to `r Table2 %>% filter(dataset == "eucalyptus", variable == "samplesize") %>% pluck("max", max)`.

```{r}
#| label: tbl-Table2
#| tbl-cap: "Mean, standard deviation and range of number of fixed and random variables, interaction terms used in models and analysis sample size (*N*). Repeated for effect-size analyses only ($Z_r$) and out-of-sample predictions only ($y_i$)."
...
The choice of variables also differed substantially among analyses (@tbl-Table3) and some analysts constructed new variables that transformed or aggregated one or more existing variables. The blue tit dataset had a total of `r filter(var_names, dataset == "blue tit") %>% pluck("variable", 1, length)` candidate variables. These variables were included in a mean of `r filter(Table3, dataset == "blue tit", subset_name == "effects") %>% pluck("mean", \(x) round(x, digits = 2))` $Z_r$ analyses (range `r filter(Table3, dataset == "blue tit", subset_name == "effects") %>% pluck("min")`- `r filter(Table3, dataset == "blue tit", subset_name == "effects") %>% pluck("max")`). The *Eucalyptus* dataset had a total of `r filter(var_names, dataset == "eucalyptus") %>% pluck("variable", 1, length)` candidate variables. The variables in the *Eucalyptus* dataset were included in a mean of `r filter(Table3, dataset == "eucalyptus", subset_name == "effects") %>% pluck("mean", \(x) round(x, digits = 2))` $Z_r$ analyses (range `r filter(Table3, dataset == "eucalyptus", subset_name == "effects") %>% pluck("min")`-`r filter(Table3, dataset == "eucalyptus", subset_name == "effects") %>% pluck("max")`).

```{r}
#| label: tbl-Table3
#| tbl-cap: "Mean,  $\\text{SE}$, minimum and maximum number of analyses in which each variable was used, for effect size analyses only ($Z_r$), out-of-sample prediction only ($y_i$), using the full dataset."
