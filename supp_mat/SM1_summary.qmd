...
calc_summary_stats_binary <- function(data, subset_name = character(1L)) {
   data %>% 
    ...
    summarise(., 
              ...
              sum_Bayesian= sum(Bayesian,na.rm=T), subset_name)
}

SubsetsBinarySummary <- tibble(data = list(Master, effects, predictions),
     subset_name = c("all", "effects", "predictions")) %>% 
  pmap(.f = calc_summary_stats_binary) %>% 
...
rewide <- SubsetsMeanSummary %>%  group_by(dataset, subset) %>% 
    pivot_longer(cols =  where(is.numeric), names_to = c(".value", "variable"), names_pattern = "(.*)_(.*)") %>%
  group_by(dataset, variable, subset)

Table2 <- rewide %>% filter(subset != "all")
