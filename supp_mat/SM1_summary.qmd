...
#Effects analyses
EffectsMeanDataSummary <- effects %>% calc_summary_stats_numeric("effects")

EffectsBinaryDataSummary<-summarise(effectsdatasetgroup, 
                                    sum_linear =sum(lm,na.rm=T), 
                                    sum_mixed= sum(mixed_model,na.rm=T),
                                    sum_Bayesian= sum(Bayesian,na.rm=T)) %>% 
  mutate(subset = "effects")

#Predictions analyses

PredsMeanDataSummary <- predictions %>% calc_summary_stats_numeric("predictions")

PredsBinaryDataSummary<-summarise(predsdatasetgroup, 
                                  sum_linear =sum(lm,na.rm=T), 
                                  sum_mixed= sum(mixed_model,na.rm=T),
                                  sum_Bayesian= sum(Bayesian,na.rm=T)) %>% 
  mutate(subset = "predictions")
...
SubsetsMeanSummary <- tibble(data = list(Master, effects, predictions),
     subset_name = c("all", "effects", "predictions")) %>% 
  pmap(.f = calc_summary_stats_numeric) %>% 
  list_rbind() #TODO create intermediate object creation e.g. 


#join  descriptive information means, sd, range
