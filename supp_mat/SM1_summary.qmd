...
sorensen_effects <- right_join(sorensen, 
                        effect_Ids, 
                        by = c("id_col")) 

sorensen_predictions <- right_join(sorensen, 
                                   predictions %>% 
                                     distinct(id_col), #TODO duplicate entry for 1-8-1 Byrock
                                     by=c("id_col")) #prediction analyses subset of diversity data

subsets_tibble_sorensen <-
  tibble(data = list(sorensen, sorensen_effects, sorensen_predictions),
         subset_name = c("all", "effects", "predictions"))
...
SorensenSummary <- 
  ...
  summarise(mean=mean(mean_diversity_index,na.rm=T),
            ...
            max=max(mean_diversity_index,na.rm=T), .groups = "keep")

#Review descriptives
ReviewData <- ManyEcoEvo_results %>% 
  ungroup %>% 
  filter(!!!filter_vars) %>% 
  select(data, -dataset) %>% 
  unnest(data) %>% 
...
#how many reviewers
reviewers <- ReviewData %>% n_distinct("ReviewerId")

#How many reviews per reviewer
ReviewerSummary <- ReviewData %>% 
  group_by(ReviewerId) %>% 
  summarise(articles = n_distinct(response_id))
