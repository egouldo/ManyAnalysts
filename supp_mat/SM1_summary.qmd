...
calculate_variable_counts <- function(data, subset_name){
  colSums(!is.na(data)) %>% 
...
Master <- ManyEcoEvo::ManyEcoEvo %>% 
  select(data) %>% 
  unnest(everything()) %>% 
  prepare_df_for_summarising()
#TODO check if drop_na results in some analyses being excluded
#TODO check whether we need to exclude analyses here or whether 'effects' is supposed to include excluded analyses or not. 
...
Table1 <- full_join(Team_Analyses,
                        SubsetsBinarySummary,
                        by = join_by(dataset, subset)) %>% 
...
diveffects <- right_join(Diversity, 
                        effect_Ids, 
                        by=c("id_col") ) #effect analysis subset of diversity data

divpredictions <- right_join(Diversity, 
                           predictions, #TODO duplicate entry for 1-8-1 Byrock
                           by=c("id_col")) #prediction analyses subset of diversity data

WholeColsums_wider <- 
  pmap(subsets_tibble_variables, calculate_variable_counts) %>% 
  list_rbind() %>% 
...
var_names <- 
  ManyEcoEvo %>% 
  pull(diversity_data) %>% 
  map(~ .x %>% select(-id_col, -dataset) %>% colnames) %>% 
  enframe("dataset", "variables")
...
BTVariables<-WholeColsums_wider[c(3:54),] %>% 
  pivot_longer(
    cols = all:predictions,
    names_to = "subset",
    values_to = "value"
