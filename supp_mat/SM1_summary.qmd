...
calc_teams_per_dataset <- function(data, subset_name){
  data %>% 
    group_by(dataset) %>% 
...
calc_analyses_per_team <- function(data, subset_name){ #NOTE this is calculating number of analyses per dataset not number of analyses per team per dataset ... 
  data %>% 
...
# Master %>% group_by(dataset) %>% 
#     count(TeamIdentifier, sort = TRUE) %>% 
#   tally(n,name = "totalanalyses") %>% 
#   mutate(subset = "all")

All_Total_Analyses <-  calc_analyses_per_team(Master, "all")

All_Teams_Per_Dataset <- calc_teams_per_dataset(Master, "all")

#How many analyst teams total
All_teams <- n_distinct(Master$TeamIdentifier) #145

#Effects eval
Effect_Analyses_Per_Team <- effects %>% 
  count(dataset, TeamIdentifier, sort = TRUE)

Effect_Total_Analyses <- calc_analyses_per_team(effects, "effects")

Effect_Teams_Per_Dataset <- calc_teams_per_dataset(effects, "effects")
...
#Predictions eval
Pred_Analyses_Per_Team <- predictions %>% 
  count(dataset, TeamIdentifier, sort = TRUE)

Pred_Total_Analyses <- calc_analyses_per_team(predictions, "predictions")

Pred_Teams_Per_Dataset <- calc_teams_per_dataset(predictions, "predictions")

#How many analyst teams in predictions analyses - pretty sure we wont need this
Pred_teams <- n_distinct(predictions$TeamIdentifier) #55 (5)
