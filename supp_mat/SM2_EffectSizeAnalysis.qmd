...
all_constructed_vars %>% 
  ...
  dplyr::mutate(included_in_yi = case_match(included_in_yi, 
                                            TRUE ~ "check", 
                                            FALSE ~ "xmark" ),
                response_variable_name = gluedown::md_code(response_variable_name)) %>% 
  group_by(dataset) %>% 
  gt::gt() %>% 
  gt::cols_label(response_variable_name = "Constructed Variable",
                 included_in_yi = gt::md("Included in $y_i$ meta-analysis?")) %>% 
