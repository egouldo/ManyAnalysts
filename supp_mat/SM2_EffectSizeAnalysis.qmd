...
For the blue tit dataset, we created a subset of analyses that excluded effects based on analyses containing highly correlated predictor variables. The list of analyses to exclude is exported internally in the package `ManyEcoEvo` and includes the following analysis ID's: `r ManyEcoEvo:::collinearity_subset %>% select(id_col) %>% gluedown::md_text()`.
