...
The anonymous `TeamIdentifier`s in the reduced subset of "expert" or "highly proficient" analysts are exported internally in the `ManyEcoEvo` package and include the Teams: `r ManyEcoEvo:::expert_subset %>% distinct(TeamIdentifier)  %>% gluedown::md_text()`.
...
For the blue tit dataset, we created a subset of analyses that excluded effects based on analyses containing highly correlated predictor variables.
Excluded analyses are exported internally in the `ManyEcoEvo` package as `ManyEcoEvo::collinearity_subset`. Analyses with the following identifiers are excluded in the reduced subset: `r ManyEcoEvo:::collinearity_subset %>% pull(id_col) %>% gluedown::md_italic()`.
